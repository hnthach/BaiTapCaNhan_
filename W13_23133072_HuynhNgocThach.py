...
# ========== HẰNG SỐ ==========
WIDTH, HEIGHT = 600, 790  # Kích thước cửa sổ
GRID_SIZE = 3             # Kích thước bảng (3x3 cho puzzle 8)
TILE_SIZE = 140           # Kích thước mỗi ô
GRID_OFFSET_X = (WIDTH - GRID_SIZE * TILE_SIZE) // 2  # Căn giữa bảng theo chiều ngang
GRID_OFFSET_Y = 70        # Vị trí bảng theo chiều dọc
FONT_SIZE = 50            # Cỡ chữ trên các ô
STEP_DELAY = 300          # Thời gian delay giữa các bước giải (ms)
BUTTON_HEIGHT = 45        # Chiều cao nút
BUTTON_WIDTH = 110        # Chiều rộng nút
BUTTON_SPACING = 8        # Khoảng cách giữa các nút
BUTTON_START_Y = HEIGHT - 220  # Vị trí bắt đầu của các nút
...
start_state = ((1, 3, 6), (5, 2, 0), (4, 7, 8))  # Trạng thái bắt đầu
...
def nondeterministic_search(start, goal, max_steps=1000, max_failures=50):
    ...
    for _ in range(max_steps):
        ...
        # Chọn bước di chuyển ngẫu nhiên
        ...
        if random.random() < 0.8:
            new_state = swap_tiles(current_state, blank_i, blank_j, ni, nj)
            if new_state not in path[-10:]:  # Tránh lặp lại trạng thái gần đây
                current_state = new_state
                path.append(current_state)
                failures = 0
            else:
...
def partial_observation_search(start, goal, max_steps=1000):
    # Chuyển start state thành list để dễ thao tác
    current_state = [list(row) for row in start]
    path = [tuple(tuple(row) for row in current_state)]  # Lưu dưới dạng tuple
    ...
    for _ in range(max_steps):
        ...
        # Xác định vùng nhìn thấy (3x3 quanh ô trống)
        visible_tiles = []
        ...
        # Tìm các ô cần di chuyển trong vùng nhìn thấy
        possible_moves = []
        for di, dj in directions:
            ni, nj = blank_i + di, blank_j + dj
            if 0 <= ni < GRID_SIZE and 0 <= nj < GRID_SIZE:
        ...
        # Ưu tiên di chuyển các ô trong vùng nhìn thấy về đúng vị trí
        best_move = None
        best_improvement = -1
        ...
        for di, dj, ni, nj in possible_moves:
            ...
            # Tính khoảng cách hiện tại và khoảng cách sau khi di chuyển
            ...
            # Ưu tiên di chuyển giúp cải thiện nhiều nhất
            if improvement > best_improvement:
                best_improvement = improvement
                best_move = (di, dj, ni, nj)
        
        # Nếu không tìm thấy cải thiện, chọn ngẫu nhiên
        ...
        if best_move:
            ...
            # Tránh lặp lại trạng thái
            if len(path) < 2 or new_state != path[-2]:
                path.append(new_state)
            else:
                # Nếu lặp lại, thử di chuyển ngẫu nhiên khác
                if possible_moves:
...
def csp_solver(start, goal, max_steps=1000):
    ...
    visited = set(path)  # Theo dõi các trạng thái đã thăm
    ...
    for _ in range(max_steps):
        ...
        # Xác định ràng buộc (ô không ở vị trí đích)
        for i in range(GRID_SIZE):
            for j in range(GRID_SIZE):
                value = current_state[i][j]
                if value != 0:
                    goal_row, goal_col = (value - 1) // GRID_SIZE, (value - 1) % GRID_SIZE
                    if (i, j) != (goal_row, goal_col):
                        constraints.append((i, j, goal_row, goal_col))

        if not constraints:  # Đã đạt goal
            return path

        # Sắp xếp ràng buộc theo khoảng cách Manhattan giảm dần
        constraints.sort(key=lambda x: -(abs(x[0] - x[2]) + abs(x[1] - x[3])))

        # Tìm ô vi phạm ràng buộc nặng nhất có thể di chuyển vào ô trống
        moved = False
        for (i, j, _, _) in constraints:
            # Kiểm tra ô này có thể di chuyển vào ô trống không
            if (abs(i - blank_i) == 1 and j == blank_j) or (abs(j - blank_j) == 1 and i == blank_i):
                # Di chuyển ô (i,j) vào vị trí trống
        ...
        if not moved:  # Không thể di chuyển hợp lệ
            return None

    return path  # Trả về đường đi dù chưa đạt goal
